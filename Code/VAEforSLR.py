...
class VAESLR(nn.Module):
    ...
    def encode(self, x):
        """
        Encoder: q(z | x) = N( z; mu_z(x),  S S^T )
    ...
    def decode(self, z):
        ...
        mu_x = z_scaled @ self.Phi.T   # => (batch_size, d)
        return mu_x
    ...
    def elbo(self, x):
        ...
        # p(x|z) = N( mu_x, gamma I )
        # -log p(x|z) = 0.5*(1/gamma)*||x - mu_x||^2 + 0.5*d*log(gamma) + const
        diff = x - mu_x
